...
let sessionUnsubs = [];
let history = [];
const HISTORY_KEY = 'agentconnect-ai-writing-history';
...
function formatRunError(err) {
  ...
  if (message.includes('Failed to connect') || message.includes('timed out')) {
    return 'AgentConnect host is not reachable. Start it with `agentconnect dev --app apps/ai-writing-assistant --ui http://localhost:5173`.';
  }
