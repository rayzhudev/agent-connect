...
export default function AIPanel({
  ...
  const finalizeResponse = useCallback(
    (rawText: string, assistantId: string) => {
  ...
  const buildContext = useCallback(() => {
    ...
    const csvLines = rows.map((row) => row.map((cell) => cell.value).join(','));
  ...
  return (
    <div className="flex flex-col h-full bg-white rounded-xl border border-slate-200 shadow-sm">
      <div className="px-4 py-3 border-b border-slate-200">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-slate-500 mt-0.5">Ask about your data</p>
          </div>
          <div className="flex items-center gap-2">
            <span
              className={`w-2 h-2 rounded-full ${isAgentConnected ? 'bg-green-500' : 'bg-slate-300'}`}
            />
      ...
      <div ref={chatWindowRef} className="flex-1 overflow-y-auto p-4 space-y-4 bg-slate-50/50">
        {messages.length === 0 ? (
      ...
      <div className="p-3 border-t border-slate-200 space-y-3">
        ...
        <div className="flex items-center justify-between">
          <button onClick={resetSession} className="text-xs text-slate-500 hover:text-slate-700">
            Reset
          </button>
          <button
