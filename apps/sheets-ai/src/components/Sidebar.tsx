    ...
    <aside className="w-64 h-full bg-slate-50 border-r border-slate-200 flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto p-2">
        ...
        {spreadsheets.length === 0 ? (
          <p className="text-sm text-slate-500 px-2 py-4 text-center">No spreadsheets yet</p>
        ) : (
            ...
            {spreadsheets.map((sheet) => (
              <li key={sheet.id}>
                ...
                >
                  ...
                  <div className="text-xs text-slate-500 mt-1">{formatDate(sheet.updatedAt)}</div>
                </div>
              </li>
            ))}
          </ul>
      ...
      {currentId && versions.length > 0 && (
        <div className="border-t border-slate-200 p-2">
          ...
          >
            ...
            <svg
              className={`w-4 h-4 transition-transform ${showVersions ? 'rotate-180' : ''}`}
              fill="none"
              viewBox="0 0 24 24"
              stroke="currentColor"
              strokeWidth={2}
