...
Token usage and context usage are separate concepts and should be emitted independently.

### Token usage

- Event type: `usage`
- Payload: `data.usage` (TokenUsage)
- A single run should emit token usage at most once per turn.
...
Standard TokenUsage keys (all optional):

- `input_tokens`: tokens sent to the model for this turn.
- `output_tokens`: tokens generated by the model for this turn.
- `total_tokens`: total tokens for this turn.
- `cached_input_tokens`: tokens served from provider cache for this turn.
- `reasoning_tokens`: provider-reported reasoning tokens, if applicable.

### Context usage

- Event type: `context_usage`
- Payload: `data.contextUsage` (ContextUsage)
- A single run should emit context usage at most once per turn.
- Context usage is about context window utilization (prompt/system/tool history), not billing.
- If only available at end, it may be emitted after `final`, or included on `final` as `contextUsage`.
- Context usage must not be merged into `usage`.

Standard ContextUsage keys (all optional):

- `context_window`: maximum context window for the active model.
- `context_tokens`: total tokens included in the context for this turn.
- `context_cached_tokens`: cached tokens included in the context.
- `context_remaining_tokens`: remaining capacity (context_window - context_tokens).
- `context_truncated`: whether input context was truncated to fit the window.
...
- Emit session streaming events with delta/final/usage/context_usage/status/message/thinking/tool_call/detail when available.
